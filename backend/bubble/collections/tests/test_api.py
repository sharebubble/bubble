...
# ---------------------------------------------------------------------------
# Tests: collection slug — generation, lookup and editing
# ---------------------------------------------------------------------------


@pytest.mark.django_db
class TestCollectionSlug:
    """Collections expose a slug that is generated by default, editable, and
    usable as an alternative lookup value on the detail endpoint."""

    def _detail_url(self, lookup):
        return reverse("api:collection-detail", kwargs={"pk": lookup})

    def test_create_generates_slug(self, owner):
        """Creating a collection without a slug generates one from the name."""
        client = APIClient()
        client.force_authenticate(owner)

        response = client.post(
            reverse("api:collection-list"),
            {"name": "Weekend Camping Gear"},
            format="json",
        )

        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["slug"] == "weekend-camping-gear"

    def test_create_with_custom_slug(self, owner):
        """A client may supply its own slug on creation."""
        client = APIClient()
        client.force_authenticate(owner)

        response = client.post(
            reverse("api:collection-list"),
            {"name": "Anything", "slug": "my-custom-slug"},
            format="json",
        )

        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["slug"] == "my-custom-slug"

    def test_retrieve_by_slug(self, collection, owner):
        """A collection can be retrieved via its slug instead of its UUID."""
        client = APIClient()
        client.force_authenticate(owner)

        response = client.get(self._detail_url(collection.slug))

        assert response.status_code == status.HTTP_200_OK
        assert response.data["id"] == str(collection.pk)
        assert response.data["slug"] == collection.slug

    def test_retrieve_by_uuid_still_works(self, collection, owner):
        """The original UUID-based lookup keeps working."""
        client = APIClient()
        client.force_authenticate(owner)

        response = client.get(self._detail_url(str(collection.pk)))

        assert response.status_code == status.HTTP_200_OK
        assert response.data["slug"] == collection.slug

    def test_retrieve_unknown_slug_returns_404(self, owner):
        client = APIClient()
        client.force_authenticate(owner)

        response = client.get(self._detail_url("does-not-exist"))
    ...
    def test_update_slug(self, collection, owner):
        ...
        response = client.patch(
            self._detail_url(str(collection.pk)),
            {"slug": "a-new-slug"},
            format="json",
        )

        assert response.status_code == status.HTTP_200_OK
        assert response.data["slug"] == "a-new-slug"
        collection.refresh_from_db()
        assert collection.slug == "a-new-slug"
